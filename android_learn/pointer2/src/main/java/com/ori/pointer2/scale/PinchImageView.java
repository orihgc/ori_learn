 ...
 * 手势图片控件
 */
public class PinchImageView extends androidx.appcompat.widget.AppCompatImageView implements IPinchImageView {
     ...
     * 外界点击事件
     */
    private OnPinchSingleClickListener mOnClickListener;

    public interface OnPinchSingleClickListener {
        void onClick(PinchImageView view, MotionEvent event);
    }

    /**
     * 外界双击事件
     */
    private OnPinchDoubleClickListener mOnDoubleClickListener;

    public interface OnPinchDoubleClickListener {
        Boolean onClick(PinchImageView view, MotionEvent event);
    }
     ...
     */
    private OnPinchLongClickListener mOnLongClickListener;

    public interface OnPinchLongClickListener {
        void onLongClick(PinchImageView view, MotionEvent event);
    }

    @Override
    public void setOnClickListener(OnClickListener l) {
        //默认的click会在任何点击情况下都会触发，所以do nothing
    }

    @Override
    public void setOnLongClickListener(OnLongClickListener l) {
        //默认的long click会在任何长按情况下都会触发，所以do nothing
    }

    @Override
    public void setOnSingleClickListener(OnPinchSingleClickListener listener) {
        this.mOnClickListener = listener;
    }

    @Override
    public void setOnLongClickListener(OnPinchLongClickListener listener) {
        this.mOnLongClickListener = listener;
    }

    @Override
    public void setOnDoubleClickListener(OnPinchDoubleClickListener listener) {
     ...
     * 拖拽缩放的限制区域
     */
    private RectF mLimitRectF = new RectF();
    ...
    private PinchModePerformanceListener mPinchPerformanceListener;

    public interface PinchModePerformanceListener {

        void onPerformance(int mode);
    }
    ...
    @Override
    public void setPinchModePerformanceListener(PinchModePerformanceListener listener) {
        this.mPinchPerformanceListener = listener;
    }
    ...
    private final GestureDetector mGestureDetector = new GestureDetector(getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (mOnLongClickListener != null) {
                mOnLongClickListener.onLongClick(PinchImageView.this, e);
            }
        }
        ...
        public boolean onDoubleTap(MotionEvent e) {
            ...
            if (mPinchMode == PINCH_MODE_SCROLL && !(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                if (mOnDoubleClickListener == null) {
                    ...
                    if (!mOnDoubleClickListener.onClick(PinchImageView.this, e)) {
                        doubleTap(e.getX(), e.getY());
                    }
                }
            }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                //停止所有动画
                cancelAllAnimator();
                //切换到滚动模式，此时切换到滚动模式并不需要notifyPinchMode，因为此时只是切换到SCROLL，但还没有执行
            ...
            //停止所有动画
            cancelAllAnimator();
            //切换到缩放模式，此时切换到缩放模式并不需要notifyPinchMode，因为此时只是切换到SCALE，但还没有执行
    ...
    private boolean scrollBy(float xDiff, float yDiff) {
        ...
        if (mLimitRectF.isEmpty()) {
            displayWidth = getWidth();
            displayHeight = getHeight();
        } else {
            displayWidth = mLimitRectF.width();
            displayHeight = mLimitRectF.height();
        }
     ...
     * 最新被通知的PinchMode
     */
    private int mLastNotifyPinchMode = mPinchMode;

    private void notifyPinchMode() {
        if (mPinchPerformanceListener != null && mLastNotifyPinchMode != mPinchMode) {
            mLastNotifyPinchMode = mPinchMode;
            mPinchPerformanceListener.onPerformance(mPinchMode);
        }
    }
}